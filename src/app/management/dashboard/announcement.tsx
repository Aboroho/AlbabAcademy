...
function Anouncement() {
  ...
  if (!notices || notices.length === 0)
    return <div className="text-sm text-gray-500">No Announcement found</div>;

  return (
    <div className="flex flex-col gap-4">
      {notices.map((notice) => (
        <div key={notice.id} className=" rounded-md ">
          <Accordion type="single" collapsible>
            <AccordionItem value={notice.id.toString()}>
              <AccordionTrigger asChild>
                <div className="flex w-full justify-between  p-2 px-4 cursor-pointer text-md bg-yellow-200">
